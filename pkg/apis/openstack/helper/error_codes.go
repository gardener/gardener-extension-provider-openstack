...
var (
	...
	dependenciesRegexp                  = regexp.MustCompile(`(?i)(PendingVerification|Access Not Configured|accessNotConfigured|DependencyViolation|OptInRequired|Conflict|inactive billing state|timeout while waiting for state to become|InvalidCidrBlock|already busy for|A resource with the ID|There are not enough hosts available|No Router found|NetworkInUse|Router interface for subnet .+ on router .+ cannot be deleted, as it is required by one or more floating IPs)`)
