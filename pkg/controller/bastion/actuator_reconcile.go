...
func (a *actuator) Reconcile(ctx context.Context, bastion *extensionsv1alpha1.Bastion, cluster *controller.Cluster) error {
	...
	err = ensureSecurityGroupRules(networkingClient, bastion, opt, securityGroup.ID)
	if err != nil {
		return err
	}
	...
	if len(networkInfo) == 0 {
		return errors.New("networkInfo must not be empty")
	}

	instance, err := ensureComputeInstance(logger, computeClient, a.bastionConfig, networkInfo[0].ID, opt)
...
func ensureComputeInstance(logger logr.Logger, client openstackclient.Compute, bastionConfig *config.BastionConfig, networkID string, opt *Options) (*servers.Server, error) {
...
func ensureSecurityGroupRules(client openstackclient.Networking, bastion *extensionsv1alpha1.Bastion, opt *Options, secGroupID string) error {
	ingressPermissions, err := ingressPermissions(bastion)
	if err != nil {
		return err
	}

	shootSecurityGroups, err := getSecurityGroups(client, opt.ShootName)
	if err != nil {
		return err
	}
